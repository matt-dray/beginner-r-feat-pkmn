...
Note that if you pass a vector to `mutate()`, the vector won't be 'recycled' for each row of your datsaet. In other words putting `mutate(new_column = c(1, 2))` won't result in a '1' in row 1, a '2' in row 3 a '1' in row 3 and so on. To do this, you can use `transform(new_column = c(1, 2))`.
